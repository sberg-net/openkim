...
public abstract class AbstractGatewayHookableCmdHandler<HookT extends org.apache.james.protocols.smtp.hook.Hook> implements CommandHandler<SMTPSession>, ExtensibleHandler {

    private List<HookT> hooks;
    private List<HookResultHook> rHooks;
    protected DefaultMetricFactory gatewayMetricFactory;

    @Override
    public Response onCommand(SMTPSession session, Request request) {
        gatewayMetricFactory = new DefaultMetricFactory(((SmtpGatewaySession) session).getLogger());
        TimeMetric timeMetric = timer(request);
        String command = request.getCommand();
        String parameters = request.getArgument();

        try (Closeable closeable = MDCBuilder
            .create()
    ...
    protected TimeMetric timer(Request request) {
        return gatewayMetricFactory.timer("smtp-" + request.getCommand().toLowerCase(Locale.US));
    }
    ...
    public static boolean canBeConvertedToSmtpAnswer(HookReturnCode returnCode) {
        return HookReturnCode.Action.ACTIVE_ACTIONS.contains(returnCode.getAction())
               || returnCode.isDisconnected();
    }
     ...
     */
    protected abstract Response doFilterChecks(SMTPSession session, String command, String parameters);
     ...
     */
    protected abstract Response doCoreCmd(SMTPSession session, String command, String parameters);
