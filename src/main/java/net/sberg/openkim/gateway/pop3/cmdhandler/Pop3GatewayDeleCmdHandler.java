...
public class Pop3GatewayDeleCmdHandler extends AbstractPOP3CommandHandler {
    private static final Collection<String> COMMANDS = ImmutableSet.of("DELE");

    private static final Logger log = LoggerFactory.getLogger(Pop3GatewayDeleCmdHandler.class);
    ...
    public Response onCommand(POP3Session session, Request request) {
        ...
        return gatewayMetricFactory.decorateSupplierWithTimerMetric("pop3-dele", () -> doDel(session, request));
    }

    private Response doDel(POP3Session session, Request request) {
        ((Pop3GatewaySession) session).log("del begins");
        if (session.getHandlerState() == POP3Session.TRANSACTION) {
            try {
                ...
                ((Pop3GatewaySession) session).getDelMsgs().add(messageId);
                ((Pop3GatewaySession) session).log("del ends");
                return DELETED;
            } catch (Exception e) {
                log.error("error on process del command", e);
                ((Pop3GatewaySession) session).log("del ends - error");
