...
public class DsnHelper {
    ...
    private static final InternetHeaders create(
        ...
        if (originalMail.getReceivedDate() != null) {
            headers.addHeader("Arrival-Date", MailUtils.RFC822_DATE_FORMAT.format(ZonedDateTime.ofInstant(originalMail.getReceivedDate().toInstant(), ZoneId.systemDefault())));
        }
    ...
    private static final InternetHeaders appendRecipient(InternetHeaders headers, Address mailAddress, String statusCode, String diagnosticCode, Date lastUpdated, String action) {
        ...
        if (lastUpdated != null) {
            headers.addHeader("Last-Attempt-Date", MailUtils.RFC822_DATE_FORMAT.format(ZonedDateTime.ofInstant(lastUpdated.toInstant(), ZoneId.systemDefault())));
        }
        return headers;
    }
}
